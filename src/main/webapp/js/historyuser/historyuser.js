...
$(document).ready(function () {
    ...
    $(document).delegate('.copy-doc-sd', 'click', function (event) {
        var code = $(this).attr('data-docno')
    ...
    $(document).delegate('.copy-doc-sa', 'click', function (event) {
        var code = $(this).attr('data-docno')

        $('#so_copy_doc_no_title').text(code);

        var username = $('#user_namex').val();
        var user_code = $('#user_code').val();
        var currentdate = new Date();
        var datetime = 'SA' + currentdate.getFullYear() + '' + (currentdate.getMonth() + 1) + '' + currentdate.getDate() + '' + currentdate.getHours() + '' + currentdate.getMinutes() + '' + currentdate.getSeconds() + '' + uuidv4().toUpperCase()
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                console.log(res)
                if (res.length > 0) {
                    $('#so_copy_doc_no').val(datetime)
                    $('#so_copy_doc_date').val(res[0].doc_date)
                    $('#so_copy_contactor').val(res[0].contactor)
                    $('#so_copy_user_creator').val(user_code + '~' + username)

                    $('#so_copy_cust_code').val(res[0].cust_code);
                    $('#so_copy_saler_code').val(res[0].saler_code).trigger('change');
                    $('#so_copy_sale_type').val(res[0].sale_type)
                    $('#so_copy_branch_code').val(res[0].branch_code + '~' + res[0].branch_name)
                    $('#so_copy_vat_type').val(res[0].vat_type)
                    $('#so_copy_remark').val(res[0].remark)
        ...
        $('#modalCopySO').modal('show');
    });

    $('.so_copy_cust_box_click').on('click', function () {
        $('#modalCopyCustSO').modal('show');
    });
    ...
    $(document).delegate('.select-copy-cust-so', 'click', function (event) {
        var code = $(this).attr('data-code')
        $('#so_copy_cust_code').val(code);
        $('#so_copy_contactor').val('');
        $('#modalCopyCustSO').modal('hide');
    });

    $('.so_copy_contactor_box_click').on('click', function () {
        var cust_code = $('#so_copy_cust_code').val();
        if (cust_code != '') {
            _searchCopyContactSO();
            $('#modalCopyContactorSO').modal('show');
        } else {
    ...
    $(document).delegate('.select-copy-contact-so', 'click', function (event) {
        var name = $(this).attr('data-name')
        $('#so_copy_contactor').val(name);

        $('#modalCopyContactorSO').modal('hide');
    });
    ...
    $('#so_btn_copy_save').on('click', function () {
        var old_doc_no = $('#so_copy_doc_no_title').text();
        var doc_no = $('#so_copy_doc_no').val();
        var doc_date = $('#so_copy_doc_date').val();
        var cust_code = $('#so_copy_cust_code').val();
        var contactor = $('#so_copy_contactor').val();
        var saler_code = $('#so_copy_saler_code').val();
        var sale_type = $('#so_copy_sale_type').val();
        var vat_type = $('#so_copy_vat_type').val();
        var vat_rate = $('#so_copy_vat_rate').val();
        var remark = $('#so_copy_remark').val();
        var branch = $('#so_copy_branch_code').val()
        var branch_code = ''
        var branch_name = ''

        var creator = $('#so_copy_user_creator').val();
        var creator_code = '';
        if (creator != '') {
        ...
        if (msg != '') {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    _getListData('');
                    setTimeout(function () {
                        $('#modalCopySO').modal('hide');
                        swal("คัดลอกรายการทำเสร็จ", {
                            icon: "success",
                        });
                    }, 500)
...
function _searchCopyContactSO() {
    var search_copy_contactor_input = $('#so_search_copy_contactor_input').val();
    var cust_code = $('#so_copy_cust_code').val();
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            for (var i = 0; i < res.length; i++) {
                ...
                html += "<li class = 'list-group-item list-group-item-action select-copy-contact-so' data-name='" + res[i].name + "' data-tel='" + res[i].telephone + "' data-address='" + res[i].address + "'> " + res[i].name + ' | ' + res[i].telephone + "|" + res[i].address + "</li>"
            }
            $('#so_list_search_copy_contact').html(html)
...
function _searchCopyCustSO() {
    var search_copy_cust_input = $('#so_search_copy_cust_input').val()
    // console.log(search_cust_input)
    ...
    $.ajax({
        ...
        success: function (res) {
            var html = '';
            for (var i = 0; i < res.length; i++) {
                html += "<li class = 'list-group-item list-group-item-action select-copy-cust-so' data-code='" + res[i].code + "' data-name='" + res[i].name_1 + "'> " + res[i].code + '~' + res[i].name_1 + " </li>"
            }
            $('#so_list_search_copy_cust').html(html)
...
function _getListData(data) {
    $.ajax({
        url: serverURL + 'getHistoryUser?search=' + data,
        method: 'GET',
        cache: false,
        success: function (res) {
...
function _showListDetail(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].trans_flag == '1001') {
            ...
            html += '        <button class="btn copy-doc-sa" style="background-color:#aa00c7;color:#fff" data-docno="' + data[i].doc_no + '"><i class="fa fa-clone"></i> คัดลอกเอกสาร</button>'
            ...
            html += '        <button class="btn copy-doc-sd" style="background-color:#aa00c7;color:#fff" data-docno="' + data[i].doc_no + '"><i class="fa fa-clone"></i> คัดลอกเอกสาร</button>'
