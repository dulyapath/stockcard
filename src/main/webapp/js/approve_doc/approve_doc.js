...
function _acceptReject() {
    var code = $('#hidden_reject_docno').val();
    var reason = $('#reject-reason').val();
    $.ajax({
        url: serverURL + 'rejectDoc',
        method: 'POST',
        data: {doc_no: code, reason: reason},
        success: function (res) {
            swal("การทำรายการทำเสร็จ", {
                icon: "success",
            });
...
function _rejeckDoc() {
    var code = $('#doc_no').val()
    $('#reject-docno').html(code)
    $('#hidden_reject_docno').val(code)
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.length > 0) {
                    ...
                    $('#contactor2').val(res[0].contactor)
...
function _getListData(data) {
    $.ajax({
        url: serverURL + 'getApproveDoc?search=' + data,
        method: 'GET',
        cache: false,
        success: function (res) {
...
function _showListDetail(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].trans_flag == '1001') {
            ...
            html += '        <button class="btn btn-success approve_doc" data-docno="' + data[i].doc_no + '"><i class="fa fa-check "></i> อนุมัติ</button>'
        } else {
            ...
            html += '        <button class="btn btn-success approve_doc_sd" data-docno="' + data[i].doc_no + '"><i class="fa fa-check "></i> อนุมัติ</button>'
        }


        html += '        <button class="btn btn-danger reject_doc"  data-docno="' + data[i].doc_no + '"><i class="fa fa-share"></i> ไม่อนุมัติ</button>'
        html += '    </div>'
        html += '</div>'
    }
